...
% talk about time and voltage value plotted on graphs. uses Charts API. chemcomp displays all three plots on one graph. clicking on the button takes to the individual graphs. All graphs are zoomable and scalable. Can click on single data point and balloon marker shows x and y values

\subsubsection{Calibration}
% mention toggle switch to change between voltage on y axis to corresponding concentration for individual plot graphs. need to know relationship between V and conc. found through calibration

% frequent calibration since sensors degrade. Include table at which for each electrochemical signal, the conc values voltages are found for. Chemcomp can insert calibration values. if calibration values aren't provided, doesnt plot a conc vs time plot

% K+ derivation

% glucose and lactate linear derivation. mention hill equation -> linear. Limitation with 3 points -> many lines of best fit -> use least sqaures method to find best line of best fit
